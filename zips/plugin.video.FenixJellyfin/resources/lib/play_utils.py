    ...
    for item in items:
        ...
        if playback_info.get("ErrorCode") is not None:
            ...
            xbmcgui.Dialog().notification(translate_string(30316),
                                          error_string,
                                          icon="special://home/addons/plugin.video.FenixJellyfin/icon.png")
            return
...
class PlaybackService(xbmc.Monitor):
    ...
    def onNotification(self, sender, method, data):
        ...
        if sender.lower() not in (
            'plugin.video.FenixJellyfin', 'xbmc', 'upnextprovider.signal'
        ):
            return
        ...
        if signal not in (
            ...
            "set_view", "plugin.video.FenixJellyfin_play_action"):
            return
        ...
        if signal in (
            "jellycon_play_action", "plugin.video.FenixJellyfin_play_action"
        ):
            play_file(play_info)
    ...
    def screensaver_deactivated(self):
        ...
        if show_change_user:
            ...
            if skip_select_user is not None and skip_select_user == "true":
                return
            xbmc.executebuiltin("RunScript(plugin.video.FenixJellyfin,0,?mode=CHANGE_USER)")
