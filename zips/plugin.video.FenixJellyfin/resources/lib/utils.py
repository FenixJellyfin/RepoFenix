...
def send_event_notification(method, data=None, hexlify=False):
    ...
    sender = 'plugin.video.FenixJellyfin'
...
def load_user_details():
    ...
    # Forçamos o username e o servidor nas definições do Kodi
    # para que o addon pense que já terminou a configuração.
    if not settings.getSetting('server_address'):
        settings.setSetting('server_address', 'https://cat.seedhost.eu/tugateamvod/jellyfin')
    window = HomeWindow()
