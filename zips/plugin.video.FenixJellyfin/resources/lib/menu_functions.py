...
def show_movie_tags(menu_params):
    ...
    for tag in tags:
        ...
        art = {"thumb": "https://i.postimg.cc/XNrJT651/Copilot-20260328-182703.png".format(ensure_text(base64.b64encode(ensure_binary(item_url))))}
...
def show_genre_list(menu_params):
    ...
    for genre in result:
        ...
        art = {"thumb": "https://i.postimg.cc/XNrJT651/Copilot-20260328-182703.png".format(ensure_text(base64.b64encode(ensure_binary(path))))}
...
def display_main_menu():
    ...
    add_menu_directory_item("Menu Fenix",
                            "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=library")
  #  add_menu_directory_item(translate_string(30407),
   #                         "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=show_custom_widgets")
    add_menu_directory_item("Pesquisa",
                            "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=addon_items")
...
def show_global_types(params):
    ...
    add_menu_directory_item(translate_string(30445),
                            "plugin://plugin.video.FenixJellyfin/?mode=GET_CONTENT&url=" + quote(continue_watching_url) +
                            "&media_type=movies" +
                            "&name_format="+quote("Episode|episode_name_format"))

    add_menu_directory_item(translate_string(30256),
                            "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=global_list_movies")
    add_menu_directory_item(translate_string(30261),
                            "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=global_list_tvshows")
...
def display_addon_menu(params):
    ...
    add_menu_directory_item("Pesquisa", "plugin://plugin.video.FenixJellyfin/?mode=SEARCH")
    add_menu_directory_item(translate_string(30435), "plugin://plugin.video.FenixJellyfin/?mode=DETECT_CONNECTION_SPEED")
    #add_menu_directory_item(translate_string(30254), "plugin://plugin.video.FenixJellyfin/?mode=SHOW_SETTINGS")
    add_menu_directory_item(translate_string(30395), "plugin://plugin.video.FenixJellyfin/?mode=CLEAR_CACHE")
    #add_menu_directory_item(translate_string(30293), "plugin://plugin.video.FenixJellyfin/?mode=CACHE_ARTWORK")
    #add_menu_directory_item("Clone default skin", "plugin://plugin.video.FenixJellyfin/?mode=CLONE_SKIN")
...
def display_tvshow_type(menu_params, view):
    ...
    url = sys.argv[0] + "?url=" + quote(path) + "&mode=GET_CONTENT&media_type=Episodes&sort=none"
    url += "&name_format=" + quote('Episode|episode_name_format')
    add_menu_directory_item("Séries - Episódios Recentes" + get_filtered_items_count_text(), url)
    ...
    # TV Show Genres
    path = "plugin://plugin.video.FenixJellyfin/?mode=GENRES&item_type=tvshow"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    #add_menu_directory_item(view_name + translate_string(30325), path)

    # TV Show Alpha picker
    path = "plugin://plugin.video.FenixJellyfin/?mode=TVSHOW_ALPHA"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
...
def display_music_type(menu_params, view):
    ...
    # genres
    path = "plugin://plugin.video.FenixJellyfin/?mode=GENRES&item_type=MusicAlbum"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    add_menu_directory_item(view_name + translate_string(30325), path)

    # Artist Alpha picker
    path = "plugin://plugin.video.FenixJellyfin/?mode=ARTIST_ALPHA"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    ...
    # Shuffle All
    path = "plugin://plugin.video.FenixJellyfin/?mode=PLAY&action=shuffle"
    if view is not None:
...
def display_movies_type(menu_params, view):
    ...
    # Genres
    path = "plugin://plugin.video.FenixJellyfin/?mode=GENRES&item_type=movie"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    add_menu_directory_item('{}{}'.format(view_name, translate_string(30325)), path)

    # Pages
    path = "plugin://plugin.video.FenixJellyfin/?mode=MOVIE_PAGES"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    #add_menu_directory_item('{}{}'.format(view_name, translate_string(30397)), path)

    # Alpha Picker
    path = "plugin://plugin.video.FenixJellyfin/?mode=MOVIE_ALPHA"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    #add_menu_directory_item('{}{}'.format(view_name, translate_string(30404)), path)

    # Years
    path = "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=show_movie_years"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    add_menu_directory_item('{}{}'.format(view_name, translate_string(30411)), path)

    # Decades
    path = "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=show_movie_years&group=true"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    #add_menu_directory_item('{}{}'.format(view_name, translate_string(30412)), path)

    # Tags
    path = "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=show_movie_tags"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
...
def display_mixed_type(params, view):
    ...
    # Mixed Genres
    path = "plugin://plugin.video.FenixJellyfin/?mode=GENRES&item_type=mixed"
    if view is not None:
        path += "&parent_id=" + view.get("Id")
    add_menu_directory_item(view_name + translate_string(30325), path)
...
def display_library_views(params):
    ...
    for view in views:
        ...
        if collection_type in view_types or item_type == "Channel":
            ...
            plugin_path = "plugin://plugin.video.FenixJellyfin/?mode=SHOW_ADDON_MENU&type=library_item&view_id=" + view.get("Id")
...
def show_widgets():    
    add_menu_directory_item("Todos os Filmes",
                            'plugin://plugin.video.FenixJellyfin/library/movies')
    
    item_limit_text = get_filtered_items_count_text()

    add_menu_directory_item(translate_string(30257) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=recent_movies')
    add_menu_directory_item(translate_string(30258) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=inprogress_movies')
    add_menu_directory_item(translate_string(30269) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=random_movies')
    add_menu_directory_item(translate_string(30403) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=movie_recommendations')

    add_menu_directory_item(translate_string(30287) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=recent_tvshows')
    add_menu_directory_item(translate_string(30455) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=random_tvshows')
    add_menu_directory_item(translate_string(30263) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=recent_episodes')
    add_menu_directory_item(translate_string(30264) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=inprogress_episodes')
    add_menu_directory_item(translate_string(30265) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=nextup_episodes')
    add_menu_directory_item(translate_string(30457) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=recent_all')
    add_menu_directory_item(translate_string(30458),
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=favorites_all')
    add_menu_directory_item(translate_string(30456) + item_limit_text,
                            'plugin://plugin.video.FenixJellyfin/?mode=WIDGET_CONTENT&type=random_all')

    xbmcplugin.endOfDirectory(int(sys.argv[1]))


def show_search():
    add_menu_directory_item(translate_string(30231), 'plugin://plugin.video.FenixJellyfin/?mode=NEW_SEARCH&item_type=Movie')
    add_menu_directory_item(translate_string(30229), 'plugin://plugin.video.FenixJellyfin/?mode=NEW_SEARCH&item_type=Series')
    add_menu_directory_item(translate_string(30235), 'plugin://plugin.video.FenixJellyfin/?mode=NEW_SEARCH&item_type=Episode')
    add_menu_directory_item(translate_string(30337), 'plugin://plugin.video.FenixJellyfin/?mode=NEW_SEARCH&item_type=Audio')
    add_menu_directory_item(translate_string(30338), 'plugin://plugin.video.FenixJellyfin/?mode=NEW_SEARCH&item_type=MusicAlbum')
    add_menu_directory_item(translate_string(30339), 'plugin://plugin.video.FenixJellyfin/?mode=NEW_SEARCH&item_type=Person')
...
def set_library_window_values(force=False):
    ...
    for item in result:
        ...
        if collection_type in ["movies", "boxsets", "music", "tvshows", "mixed"]:
            ...
            # plugin.video.FenixJellyfin-
            prop_name = "view_item.%i.name" % index
            home_window.set_property(prop_name, name)
            log.debug("set_library_window_values: plugin.video.FenixJellyfin-{0}={1}".format(prop_name, name))

            prop_name = "view_item.%i.id" % index
            home_window.set_property(prop_name, item_id)
            log.debug("set_library_window_values: plugin.video.FenixJellyfin-{0}={1}".format(prop_name, item_id))

            prop_name = "view_item.%i.type" % index
            home_window.set_property(prop_name, collection_type)
            log.debug("set_library_window_values: plugin.video.FenixJellyfin-{0}={1}".format(prop_name, collection_type))

            thumb = get_art_url(item, "Primary", server=server)
            prop_name = "view_item.%i.thumb" % index
            home_window.set_property(prop_name, thumb)
            log.debug("set_library_window_values: plugin.video.FenixJellyfin-{0}={1}".format(prop_name, thumb))
