...
def zip_files(py_version: str, source: str, target: str, dev: bool) -> None:
    ...
    """
    archive_name = f'plugin.video.FenixJellyfin+{py_version}.zip'
        ...
        for root, dirs, files in os.walk(args.source):
            for filename in filter(file_filter, files):
                ...
                if dev or folder_filter(file_path):
                    relative_path = os.path.join('plugin.video.FenixJellyfin', os.path.relpath(file_path, source))
...
def file_filter(file_name: str) -> bool:
    ...
    """
    return (
        not (file_name.startswith('plugin.video.FenixJellyfin') and file_name.endswith('.zip'))
